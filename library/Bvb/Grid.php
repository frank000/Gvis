...
abstract class Bvb_Grid {
    ...
    public function setAction($array = array()){
        if(!empty($array)){
            ...
            if($num == 1 ){
                           //Zend_Debug::dump($array[0]);
                 $right = new Bvb_Grid_Extra_Column();
                 $right->position('right')->name('unique_name')->title('Ações')->decorator($array[0]);


                 $this->addExtraColumns($right);   
            } else if($num > 1) {
                foreach ($array as $value){
                    $action .= $value;
                }
                 $right = new Bvb_Grid_Extra_Column();
                 $right->position('right')->name('unique_name')->title('Ações')->decorator($action);
